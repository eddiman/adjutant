...
Read the file `~/.adjutant/PAUSED`. If it exists, output exactly "Adjutant is paused. Skipping pulse." and stop immediately. Do nothing else.
...
Read `~/.adjutant/knowledge_bases/registry.yaml` to get the list of all registered knowledge bases.
...
```bash
bash ~/.adjutant/scripts/capabilities/kb/query.sh "<name>" "Quick pulse: what is the current status? List any active blockers, open items, or upcoming deadlines in the next 2 weeks. Be brief — bullet points only."
```
...
Append an entry to today's journal file at `~/.adjutant/journal/YYYY-MM-DD.md` (create it if it doesn't exist). Use the current time. Format:
...
- Write the insight to `~/.adjutant/insights/pending/YYYY-MM-DD-HHMM.md` with:
