...
llm:
  ...
  models:
    ...
    medium: "anthropic/claude-sonnet-4-5"   # escalations
...
The OpenCode workspace configuration. This file controls which files Adjutant is allowed to read and write. It is generated by the setup wizard and you generally don't need to edit it.

The key section is the permissions block, which restricts Adjutant to `~/.adjutant/` and denies access to `.env` directly:
...
If you want to allow Adjutant to read files outside `~/.adjutant/` (for example, to read a project's README), add those paths to the `allow` list — but think carefully before doing so. The default restriction is intentional.
