...
@test "send: msg_send_text sends a correctly-formed Telegram API call" {
...
@test "send: msg_send_text includes reply_to_message_id when provided" {
  msg_send_text "Reply to this" "42"
  assert_mock_args_contain "curl" "reply_to_message_id=42"
}

@test "send: msg_send_text accepts messages up to 5000 characters without error" {
  local long_msg
...
@test "send: msg_send_photo sends a correctly-formed multipart API call" {
...
@test "send: msg_send_photo returns 1 and does not call curl when file does not exist" {
...
@test "send: msg_react sends a correctly-formed reaction API call" {
  msg_react "12345" "👍"
...
@test "send: msg_typing start/stop lifecycle creates and removes a pidfile" {
...
@test "send: msg_typing stop is safe to call when no typing indicator is active" {
...
@test "send: msg_authorize returns 0 when from_id matches TELEGRAM_CHAT_ID" {
  run msg_authorize "99999"
  assert_success
}

@test "send: msg_authorize returns 1 when from_id does not match TELEGRAM_CHAT_ID" {
